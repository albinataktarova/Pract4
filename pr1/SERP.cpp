...
Serpent_Cryptor::Serpent_Cryptor(const string& Input, const string& Output, const string& Pass)
{
    FileIn = Input;
    FileOut = Output;
    psw = Pass;
...
bool Serpent_Cryptor::Serpent_Encrypt ()
{
    ...
    pull.write((char*)iv, Serpent::BLOCKSIZE);
    pull.close();

    cout << "\nIV Successfully created: " << FileOut << ".iv" << endl;

    try {
        CBC_Mode<Serpent>::Encryption encr;
...
bool Serpent_Cryptor::Serpent_Decrypt ()
{
    SecByteBlock key(Serpent::DEFAULT_KEYLENGTH);
    PKCS12_PBKDF<SHA512> pbkdf;
    pbkdf.DeriveKey(key.data(), key.size(), 0, (byte*)psw.data(), psw.size(), (byte*)salt.data(), salt.size(), 1024, 0.0f);
    ...
    if (pool.good()) {
        pool.read((char*)&iv, Serpent::BLOCKSIZE);
        pool.close();
    }
    ...
    try {
        CBC_Mode<Serpent>::Decryption decr;
